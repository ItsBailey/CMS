...
// Veranders je wachtwoord.
if(isset($_POST['submit'])) {
    // Defineer de ingevoerde gegevens.
    $passOld = htmlspecialchars($_POST['oldpassword']);
    $passNew = htmlspecialchars($_POST['newpassword']);
    $passNew2 = htmlspecialchars($_POST['newpassword2']);
    // Is het huidige wachtwoord niet hetzelfde als in de database? Error.
    if(Users::hashPass($passOld) !== Users::getData('password', $userID)) {
        $template->assign('editError', incWeb::redError('Oud wachtwoord fout'));
    // Zijn de nieuwe wachtworden niet hetzelfde? Error.
    } else if($passNew != $passNew2) {
        $template->assign('editError', incWeb::redError('Wachtwoorden niet hetzelfde'));
    // Is het nieuwe wachtwoord langer dan 32 of korter dan 6 karakters? Error.
    } else if(strlen($passNew) < 6 || strlen($passNew) > 32) {
        $template->assign('editError', incWeb::redError('Wachtwoord te lang/kort'));
    // Alles goed? Verander het wachtwoord en geef een error.
    } else {
        Users::updateUser($userID, "password", Users::hashPass($passNew));
        $template->assign('editError', incWeb::greenError('Wachtwoord met succes veranderd'));
    }
}

// Basis assigns voor de pagina.
$varSettingsPassword = array(
    "siteTitle" => $siteShort." &bull; Wachtwoord wijzigen",
...
$template->assign($varSettingsPassword);

// Maak de pagina.
$template->draw($siteTemplate.'_header');
$template->draw($siteTemplate.'SettingsPassword');
