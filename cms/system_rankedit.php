...
if(isset($_POST['submitChoose'])) {
    $level = htmlspecialchars($_POST['rankid']);
    Core::forcePage('?level='.$level);
}

// Basis assigns voor de pagina.
$varCmsSystem = array(
    "siteTitle" => $cmsName."CMS - Rangen wijzigen"
);

// Defineer de pagina assigns.
$template->assign($varCmsSystem);

// Maak de pagina.
$template->draw($cmsTemplate.'_system');
$template->draw($cmsTemplate.'cmsSystemRankEdit');
